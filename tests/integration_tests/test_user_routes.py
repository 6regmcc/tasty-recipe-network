...
def test_create_user_required_fields(test_client):
    body = {"password": "Password1", "first_name": "Tom", "last_name": "Smith"}
    response = test_client.post("/user/create_user", json=body)
    assert response.status_code == 422
    body = {"username": "tom2@email.com", "first_name": "Tom", "last_name": "Smith"}
    response2 = test_client.post("/user/create_user", json=body)
    assert response2.status_code == 422


def test_create_user_optional_fields(test_client):
    body = {"username": "tom3@email.com", "password": "Password1"}
    response = test_client.post("/user/create_user", json=body)
    assert response.status_code == 200
...
def test_login_success(test_client, db_session, create_user_fixture):
    ...
    assert user["user_id"]
    body = {"username": user["username"], "password": "Password1"}

    response = test_client.post("user/token", data=body)
    data = response.json()
    assert "access_token" in data
    payload = jwt.decode(
        data["access_token"], os.getenv("SECRET_KEY"), algorithms=["HS256"]
    )
...
def test_incorrect_password(test_client, create_user_fixture):
    user = create_user_fixture
    body = {"username": user["username"], "password": "Password2"}
    response = test_client.post("user/token", data=body)
    data = response.json()
...
def test_incorrect_username(test_client, create_user_fixture):
    user = create_user_fixture
    body = {"username": "tom6@gmail.com", "password": user["password"]}
