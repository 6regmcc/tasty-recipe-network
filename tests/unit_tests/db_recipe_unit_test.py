...
def test_create_ingredients(return_ingredients, create_recipe, db_session):
    ...
    added_ingredients = db_create_recipe_ingredients(
        ingredients=ingredients_to_add, recipe_id=new_recipe["recipe_id"], db=db_session
    )
...
def test_create_recipe_with_ingredients_exception(
    recipe_one, create_user_fixture, db_session, mocker
):
    new_recipe = recipe_one
    user_id = create_user_fixture["user_id"]
    mock_db_delete = Mock()
    ...
    mocker.patch(
        "tasty_recipe_network.db.db_recipes.db_create_recipe_ingredients",
        side_effect=ValueError("Test exception handling"),
    )
...
def test_delete_recipe(create_recipe_with_ingredients, db_session):
    ...
    assert (
        ...
        .filter(Ingredient.ingredient_id == example_ingredient_id)
        .one()
    )
    delete_recipe(recipe_id=recipe_to_delete.recipe_id, db=db_session)
    found_recipe = (
        db_session.query(Recipe).filter(Recipe.recipe_id == example_recipe_id).first()
    )
...
def test_delete_ingredient(create_recipe_with_ingredients, db_session):
    ...
    assert db_session.query(Ingredient).filter(
        Ingredient.ingredient_id == example_ingredient_id
    )
...
def test_get_recipe_id_from_ingredient_id_success(
    create_recipe_with_ingredients, db_session
):
    new_recipe = create_recipe_with_ingredients
    example_ingredient_id = new_recipe.ingredients[0].ingredient_id
    found_recipie_id = db_get_recipe_id_from_ingredient_id(
        ingredient_id=example_ingredient_id, db=db_session
    )
...
def test_add_ingredient_to_recipe(create_recipe_with_ingredients, db_session):
    ...
    added_ingredient = add_ingredient_to_recipe(
        new_ingredient=new_ingredient, recipe_id=recipe_id, db=db_session
    )
...
def test_edit_ingredient(create_recipe_with_ingredients, db_session):
    ...
    updated_ingredient = db_edit_ingredient(
        ingredient=ingredient_update_data, ingredient_id=ingredient_id, db=db_session
    )
...
def test_edit_ingredient_failure(db_session):
    ...
    with pytest.raises(sqlalchemy.exc.NoResultFound):
        db_edit_ingredient(
            ingredient=ingredient_update_data,
...
def test_db_edit_recipe(create_recipe_with_ingredients, db_session):
    ...
    update_recipe_data = UpdateRecipe(
        title="Updated title", is_vegan=True, is_vegetarian=True, body="Updated Body"
    )
    ...
    assert (
        updated_recipe.ingredients[0].ingredient_name
        == recipe_to_update.ingredients[0].ingredient_name
    )
...
def test_check_if_user_owns_recipe(create_user_fixture, recipe_one, db_session):
    ...
    assert db_check_if_user_owns_recipe(
        recipe_id=recipe1.recipe_id, user_id=user_1["user_id"], db=db_session
    )
...
def test_check_if_user_owns_recipe_failure(create_user_fixture, db_session):
    ...
    with pytest.raises(sqlalchemy.exc.NoResultFound):
        db_check_if_user_owns_recipe(
            recipe_id=000000000, user_id=user_1["user_id"], db=db_session
        )
